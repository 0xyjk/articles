---
name: publish-draft
description: 将 Markdown 文件发布到微信公众号草稿箱。当用户说"发布到公众号"、"推送到微信"、"上传到草稿箱"、"publish to wechat"、"publish-draft"，或者提到微信公众号文章发布时使用。输入是 Markdown 文件，自动转换为微信兼容 HTML、上传图片、调用草稿箱 API。
---
...
1. 项目级：`.publish-draft/.env`（当前工作目录下）
2. 用户级：`~/.publish-draft/.env`
...
若无配置文件，引导用户创建：
```bash
mkdir -p .publish-draft
echo "WECHAT_APP_ID=\nWECHAT_APP_SECRET=" > .publish-draft/.env
```
...
检查配置：
```bash
test -f .publish-draft/.env && cat .publish-draft/.env || \
test -f ~/.publish-draft/.env && cat ~/.publish-draft/.env || \
echo "未找到配置文件"
```
...
发布成功后，将文章从草稿目录移至已发布目录：

```bash
# 确保目标目录存在
mkdir -p posts/<series>/

# 移动文章
mv drafts/<series>/<NN>-<slug>.md posts/<series>/<NN>-<slug>.md
```

若仓库使用 git 管理，优先用 `git mv`：
```bash
mkdir -p posts/<series>/
git mv drafts/<series>/<NN>-<slug>.md posts/<series>/<NN>-<slug>.md
```
